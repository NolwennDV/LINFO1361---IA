...
def simulated_annealing(problem, schedule=exp_schedule(), stopping_criterion=None):
    ...
    for t in range(sys.maxsize):
        try:
            ...
            if T == 0:
                return current.state
            if stopping_criterion is not None and stopping_criterion == problem.value(current.state):
                return current.state
            neighbors = current.expand(problem)
            if not neighbors:
                return current.state
